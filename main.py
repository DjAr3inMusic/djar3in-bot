...
await update.message.reply_text("🎤 برای رزرو، لطفاً اطلاعات زیر رو برام بفرست:\n""- تاریخ مراسم\n- محل برگزاری\n- نوع مراسم\n- شماره تماس")


def search_itunes(query: str, limit: int = 5):url = "https://itunes.apple.com/search"params = {"term": query, "media": "music", "limit": limit}
try:
...
if not ok:
await status_msg.edit_text("😔 نتونستم ویدیو رو دانلود کنم. ممکنه لینک خصوصی یا نامعتبر باشه.")
return
...
if not audio_file:break
await status_msg.edit_text("😔 صوتی پیدا نشد.")
return
result = recognize_song_with_audd(audio_file)

if not result:
await status_msg.edit_text("😔 نتونستم آهنگ رو تشخیص بدم. شاید موزیک تو ریلز واضح نبود.")
return
...
if spotify.get("external_urls", {}).get("spotify"):
            buttons.ap[InlineKeyboardButton("🟢 Spotify", url=spotify["external_urls"]["spotify"])])
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):query = update.message.text.strip()
if not query:return

if INSTAGRAM_LINK_PATTERN.search(query):await handle_instagram_link(update, context, query)return

await update.message.reply_text(f"🔍 در حال جستجوی «{query}»...")

results = search_itunes(query)
if not results:
await update.message.reply_text("متأسفانه چیزی پیدا نکردم 😔 لطفاً اسم دقیق‌تری امتحان کن.")

for track in results:return
